...
Each Gold flow independently populates its own JSONB result column(s) so "Gold status" for a document is derived from which analytical result columns are non-NULL.
"""
...
# Gold analytical modules : {"Module identifier by API" : "corresponding column in gold.analytics"}
GOLD_MODULES = {
    "lexical": "lex_metrics",
...
def _gold_status_for_row(gold_row: Optional[tuple]) -> tuple[List[GoldModuleStatus], str]:
    ...
    done_count = sum(module.done for module in modules)
...
def get_status(doc_id: int):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT status_processing, error_message
                FROM silver.text
                WHERE doc_id = %s
                """,
                (doc_id,),
            ...
            cur.execute(
                f"""
                SELECT {", ".join(GOLD_MODULES.values())}
                FROM gold.analytics
                WHERE doc_id = %s
                """,
                (doc_id,),
...
def get_all_statuses():
    ...
    try:
        with conn.cursor() as cur:
            ...
            doc_ids = [row[0] for row in base_rows]
            gold_by_doc = {}

            if doc_ids:
                cur.execute(
                    f"""
                    SELECT doc_id, {", ".join(GOLD_MODULES.values())}
                    FROM gold.analytics
                    WHERE doc_id = ANY(%s)
                    """,
                    (doc_ids,),
    ...
    for doc_id, status_processing, error_message in base_rows:
        gold_modules, gold_status = _gold_status_for_row(
